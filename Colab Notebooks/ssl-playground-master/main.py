...
def main(args):
    ...
    if args.load:
        ...
        trainer = pl.Trainer(default_root_dir=log_dir, max_epochs=args.ssl_epochs,enable_progress_bar=False, gpus=gpus,callbacks=[TQDMProgressBar(refresh_rate=100)])
    ...
    trainer = pl.Trainer(max_epochs=args.clf_epochs, gpus=gpus,enable_progress_bar=False,callbacks=[TQDMProgressBar(refresh_rate=100)])
    trainer.fit(
        classifier,
