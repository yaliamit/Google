...
class SimSiam(pl.LightningModule):
    def __init__(self, backbone, backbone_out_dims, loss=None):
        super().__init__()
        self.backbone = backbone
        self.projection_head = SimSiamProjectionHead(backbone_out_dims, 2048, 2048)
    ...
    def configure_optimizers(self):
        optim = torch.optim.SGD(self.parameters(), lr=6e-2,
...
class DirectCopyBP(pl.LightningModule):
    def __init__(self, backbone, backbone_out_dims, loss=None, symmetric_loss=True,
                 mu=0.5, epsilon=0.3, hidden_size=256, feature_size=64):
        super().__init__()
        self.backbone = backbone
        self.projection_head = BYOLBPProjectionHead(backbone_out_dims, hidden_size, feature_size)
        self.mu = mu
        self.epsilon = epsilon
        ...
        if not loss:
            ...
            self.criterion = loss
        self.register_buffer('corr_matrix', torch.zeros(feature_size, feature_size))
        self.register_buffer('eye', torch.eye(feature_size))

        self.history_size = 2048
        self.z0_history = torch.zeros(self.history_size, feature_size)
        self.cur_idx = 0
...
class LinearProbingClassifier(pl.LightningModule):
    def __init__(self, backbone, backbone_out_dims):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        for s,t in self.backbone.items():
            print(s)
            print(t)
        return loss
